  ...
  def collective_all_gather():
    ...
    for d in range(num_devices):
      with ops.device(devices[d]):
        ...
        #	print(tensor.name,",",tensor)
        gradients_shape = graph.get_tensor_by_name(input_tensors[d].name).get_shape( ).as_list()
        #获取batch_size,获取不到取默认值
        batch_size = global_batch_size #全局变量
        # print("******batch size=",batch size)
        #区分是values还是indices,每个批次里更新的单变量的个数不会超过batchsize个，因此将其补齐为batch_size
        if len(gradients_shape) == 2:
          input_tensors[d] = tf.pad(input_tensors [d],[[0,batch_size],[0,0]])[:batch_size,:]
        if len(gradients_shape) == 1:
          input_tensors[d] = tf.pad(input_tensors [d],[[0,batch_size]l)[:batch_size]
          gather_op = collective_ops.all_gather(input_tensors[d],group_size,group_key, instance_key)
          out_tensors.append(gather_op)
    return out_tensors
